 ...
 *
 * Edited by
 * NAMES: Joshua Eckles, Nate Nordquist
 *
 */
 ...
 */
void
dbe_print(struct db_entry* entry, FILE* stream)
{
  //DONE: Modify this function to have an additional FILE* parameter.
...
do_export_db(struct db_entry** db, const char* filename)
{
  ...
  int i;
  for (i=0; i < db_count_entries(db); i++) {
    dbe_print(db[i], file);
  }
  fclose(file);
}
