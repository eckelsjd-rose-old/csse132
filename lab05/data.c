...
dbe_alloc(const char* name, const char* value)
{
  ...
  struct db_entry* structpointer = malloc(sizeof(struct db_entry));
  char* namepointer = malloc(strlen(name) + 1);
  char* valuepointer = malloc(strlen(value) + 1);
  structpointer[0].name = namepointer;
  structpointer[0].value = valuepointer;
  memcpy(namepointer, name, strlen(name) + 1);
  memcpy(valuepointer, value, strlen(value) + 1);
            ...
            int initialIndex)
{
  ...
  // possibilities.
  struct db_entry* currentpointer;
  int i = initialIndex;
  char* currentname;
  while(database[i] != 0) {
    currentpointer = database[i];
    currentname = currentpointer->name;
    if(strncmp(target, currentname, strlen(target)) == 0) {
      return i;
    }
    i++;
  }
...
do_remove_first_match(struct db_entry** db, const char* target)
{
  ...
  int index = db_find_one(db, target, 0);
  if (index != -1) {
    db_remove(db,index);
    return 1;
  }
